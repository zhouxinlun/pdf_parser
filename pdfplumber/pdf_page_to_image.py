...
"""
PDF页面提取器 (使用pdfplumber和pypdfium2)
...
class PDFPageToImage:
            ...
            try:
                ...
                if metadata:
                    if 'Title' in metadata:
                        self.pdf_info["标题"] = metadata['Title']
                    if 'Author' in metadata:
                        self.pdf_info["作者"] = metadata['Author']
                    if 'CreationDate' in metadata:
                        self.pdf_info["创建日期"] = self._format_date(metadata['CreationDate'])
                    if 'ModDate' in metadata:
                        self.pdf_info["修改日期"] = self._format_date(metadata['ModDate'])
            except:
                pass
        
        # 使用pypdfium2打开PDF用于渲染
        self.pdf_document = pdfium.PdfDocument(pdf_path)
    ...
    def extract_page(self, page_num, output_path, dpi=300, output_format="png"):
        ...
        # 获取页面
        pdf_page = self.pdf_document[page_num - 1]
...
def main():
    ...
    try:
        # 创建PDF页面提取器
        extractor = PDFPageToImage(args.pdf_path)
